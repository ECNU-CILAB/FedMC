...
class SERVER:
    ...
    def setup_clients(self):
        users, train_loaders, test_loaders = setup_datasets(dataset=self.config.dataset,
                                                            ...
                                                            num_users=self.config.numUsers,
                                                            alpha=self.config.alpha,
                                                            classes_per_client=self.config.classesPerClient)
        clients = [
    ...
    def federate(self):
        ...
        for c in self.clients:
            print(c.user_id, c.dataset_stats)
        ...
        for i in tqdm(range(self.config.numRounds)):
            ...
            for k in range(len(self.selected_clients)):
                ...
                train_samples_num, update, loss = surrogate.train(round_th=i)
                # c <-- surrogate
                c.update(surrogate)
                self.updates.append((train_samples_num, copy.deepcopy(update), c.dataset_stats))
