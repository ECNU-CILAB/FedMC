...
class SERVER:
    ...
    def federate(self):
        ...
        for i in tqdm(range(self.config.numRounds)):
            ...
            for k in range(len(self.meta_train_clients)):
                surrogate = self.surrogates[k]
                c = self.meta_train_clients[k]
                # surrogate <-- c
                surrogate.update(c)
                surrogate.set_shared_critic_params(self.params)
                trainSamplesNum, update, loss = surrogate.meta_train(round_th=i)
                # c <-- surrogate
                c.update(surrogate)
            ...
            for k in range(len(self.meta_test_clients)):
                surrogate = self.surrogates[k]
                c = self.meta_test_clients[k]
                # surrogate <-- c
                surrogate.update(c)
                surrogate.set_shared_critic_params(self.params)
                trainSamplesNum, update, loss = surrogate.meta_test(round_th=i)
                # c <-- surrogate
                c.update(surrogate)
                self.updates.append((trainSamplesNum, copy.deepcopy(update)))
